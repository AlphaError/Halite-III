...
while True:
    ...
    for ship in me.get_ships():
        ...
        if ship.is_full or ship.halite_amount >= 300+(game_map.calculate_distance(ship.position, close_drop.position)*20)+(game.turn_number*2):
            ...
            shipyard_dist = int(3+game.turn_number/200)   #**
            ...
            if dist < shipyard_dist:
                ...
                if game_map.calculate_distance(ship.position, close_drop.position) > (shipyard_dist+2) and (len(me.get_ships()) / (len(me.get_dropoffs())+1)) >= 5 and me.halite_amount > 4500:
                ...
                elif game_map[ship.position].halite_amount <= 30+(game.turn_number/7.5):
                    good_moves = []
                    for width in range(game_map.width):
                        for height in range(game_map.height):
                            ...
                            if game_map.calculate_distance(pos, close_drop.position) < (shipyard_dist+1):
                                ...
                                if game_map[pos].is_occupied or game_map[pos].has_structure:
                                    pass
                                else:
                                    if game_map.calculate_distance(ship.position, pos) > 4:
                                        pass
                    ...
                    if len(good_moves) == 0:
                        ...
                        for pos2 in good_moves:
                            if game_map[pos2].halite_amount > game_map[move].halite_amount:
                                move = pos2
    ...
    if me.halite_amount-constants.SHIP_COST > (game.turn_number*2) and game.turn_number < 300 and me.halite_amount >= constants.SHIP_COST and not game_map[me.shipyard].is_occupied:
